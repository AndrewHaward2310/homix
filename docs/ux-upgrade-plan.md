...
- ✅ **C2 (ĐÃ LÀM)** — Bản SALE ở `/agent/collections` (tab "Báo giá"): tách lõi builder thành `components/combo/combo-builder-core.tsx` (render-prop `renderActions`, state trên URL, dùng chung `priceCombo`); trang khách `/combo/tu-thiet-ke` và công cụ báo giá cùng một lõi (DRY). Sale gắn lead (tùy chọn), "Sao chép link đặt" (link tuyệt đối `/property/[id]?nights=&guests=&perks=`) + "Sao chép nội dung báo giá" (text gửi Zalo/email). Verify UI: giá gói khớp 4.692.000đ, gợi ý bậc kế, badge tiết kiệm. Review 2 tầng (Codex + subagent): 0 HIGH/MEDIUM; fix LOW (icon Sparkles ở badge, select value khớp option async, lỗi tải lead có retry). `tsc` sạch, i18n vi/en parity 450 key.
- ✅ **C3 (ĐÃ LÀM)** — 3 lối vào builder + gợi ý thông minh:
  - **Wizard hỏi nhanh** (`components/combo/combo-wizard.tsx`): modal đi-với-ai/mấy-đêm/thích-gì → dựng sẵn n/g/perks (map sở thích→perk id ưu tiên) áp vào URL builder. A11y: Escape, focus vào modal + trap + restore, khoá cuộn nền; loading/error + retry, chặn "Dựng" khi perk chưa tải.
  - **Mở từ combo mẫu**: trang `/combo/[id]` thêm nút "Tùy chỉnh combo này" (`buildBuilderLink` → builder có sẵn) và sửa CTA "Đặt combo" mang trải nghiệm sang trang đặt (trước trỏ trơn `/property/[id]` nên mất combo).
  - **Gợi ý thông minh** (`lib/combo-suggest.ts`): học đồng-xuất-hiện perk từ `COMBO_DEFS` → chip "hay đi cùng" trong builder; verify: chọn vé biển hồ ⇒ gợi ý BBQ/bữa sáng/kayak đúng thứ hạng.
  - Review 2 tầng (Codex + subagent): fix **HIGH** (link `/property` bỏ nights/guests chết — số đêm do lịch); MEDIUM (wizard async, a11y modal, map perk-id thay vì category); LOW (encode URL bằng `URLSearchParams`, role=dialog trên panel). `tsc` sạch, i18n vi/en parity 467.
...
- ✅ **(iteration 9) Concept C — tạp chí**: `LifestyleSection` viết lại thành **panel FULL-BLEED** — ảnh tràn tới mép màn hình, chữ cỡ tạp chí (`md:text-[2.5rem]`), so le trái/phải, đánh số 01–03 (ba chương là chuỗi một-ngày). Semantics `<ol>/<li>` truyền tải thứ tự, số chỉ là trang trí (`aria-hidden`), `alt=""` cho ảnh minh hoạ (không đọc trùng tiêu đề). Review Codex: fix **padding ngược chiều** (khối chữ nay dồn về đường ráp với ảnh, `max-w-xl` để margin-auto có tác dụng), tương phản số, alt lặp. Không có scroll ngang. *(Lượt này chỉ có Codex — subagent bị dừng do hết hạn mức phiên.)*
- **Còn lại của C (tuỳ chọn)**: dải cuộn ngang scroll-snap cho "Căn hộ nổi bật" — hiện GIỮ bố cục bento đã được duyệt ở iteration 7, chưa thay.
